...
export default function Venues() {
  ...
  const handleDelete = async (v) => {
    if (!window.confirm(`Удалить заведение "${v.name}"? Это удалит все связанные данные.`)) return;
    await api.deleteVenue(v.id).catch(e => alert(e.message));
    load();
  };
  ...
  return (
    <>
      <div className="page-header">
        <h1>Заведения <span className="tag">{venues.length}</span></h1>
        <button className="btn btn-primary" onClick={openCreate}>+ Добавить</button>
      </div>

      <div className="filter-bar">
        <input className="search-input" placeholder="Поиск по названию, категории, адресу..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      <div className="card">
        <div className="table-wrap">
          {loading ? <div className="loading">Загрузка...</div> : (
            <table>
              <thead><tr><th>Заведение</th><th>Категория</th><th>Адрес</th><th>Часы работы</th><th>Рейтинг</th><th>Статус</th><th>Действия</th></tr></thead>
              <tbody>
                {filtered.map(v => (
                  <tr key={v.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        {v.image_url && <img src={v.image_url} alt="" style={{ width: 40, height: 40, borderRadius: 8, objectFit: 'cover' }} />}
                        <div>
                    ...
                    <td className="truncate text-sm">{v.location}</td>
                    <td className="text-sm">{v.open_time} – {v.close_time}</td>
                    <td><span style={{ fontWeight: 700 }}>⭐ {v.rating || 0}</span> <span className="text-muted text-sm">({v.review_count || 0})</span></td>
                    ...
                    <td>
                      <div className="flex gap-2">
                        ...
                        <button className="btn btn-danger btn-sm" onClick={() => handleDelete(v)}>🗑️</button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      {modal !== null && (
        <Modal title={modal === 'create' ? 'Новое заведение' : 'Редактировать заведение'} onClose={closeModal} onSave={handleSave} saving={saving}>
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              ...
              <input className="form-input" value={form.category} onChange={e => set('category', e.target.value)} placeholder="Billiards, Bowling..." />
            </div>
          </div>
          <div className="form-group">
          ...
          <div className="form-row">
            <div className="form-group">
              ...
              <input className="form-input" type="time" value={form.open_time || '10:00'} onChange={e => set('open_time', e.target.value)} />
            </div>
            <div className="form-group">
              <label className="form-label">Закрытие</label>
              <input className="form-input" type="time" value={form.close_time || '22:00'} onChange={e => set('close_time', e.target.value)} />
            </div>
          </div>
          <div className="form-row">
            <div className="form-group">
          ...
          {modal !== 'create' && (
            <div className="form-group flex items-center gap-2">
              ...
              <span className="form-label" style={{ marginBottom: 0 }}>Активно</span>
            </div>
          )}
        </Modal>
      )}
